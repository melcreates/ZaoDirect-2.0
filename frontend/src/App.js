...
// Images
import zaoIcon from "assets/images/ZaoIcon.svg";
...
export default function App() {
  ...
  const isRouteAllowed = (route) => {
    ...
    return authContext.userRole && route.allowedRoles.includes(authContext.userRole);
  };

  const filterVisibleRoutes = (allRoutes) =>
    allRoutes
      .map((route) => {
        if (route.collapse) {
          const nested = filterVisibleRoutes(route.collapse);
          if (nested.length === 0) return null;
          return { ...route, collapse: nested };
        }
        if (route.type === "auth") return route;
        if (!isRouteAllowed(route)) return null;
        return route;
      })
      .filter(Boolean);

  const visibleRoutes = useMemo(() => filterVisibleRoutes(routes), [authContext.userRole]);
