...
function ShipmentTracking() {
  ...
  const loadEvents = async (batchId = "") => {
    const path = batchId ? `/api/ops/shipment-events?batchId=${batchId}` : "/api/ops/shipment-events";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [batchData] = await Promise.all([HttpService.get("/api/ops/batches"), loadEvents()]);
        if (!mounted) return;
  ...
  const onSubmit = async (e) => {
    ...
    setNotice("");
    try {
      await HttpService.post("/api/ops/shipment-events", {
