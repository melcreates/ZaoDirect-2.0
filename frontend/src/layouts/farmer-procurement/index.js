...
function FarmerProcurement() {
  ...
  const loadRows = async () => {
    const data = await HttpService.get("/api/ops/farmer-purchase-orders");
  ...
  const allocateToBatch = async () => {
    ...
    setDialogSuccess("");
    try {
      await HttpService.post("/api/ops/batch-items", {
        batchId,
  ...
  const createBatchAndAllocate = async () => {
    ...
    setDialogSuccess("");
    try {
      const created = await HttpService.post("/api/ops/batches", {
