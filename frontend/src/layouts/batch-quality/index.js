...
function BatchQuality() {
  ...
  const loadBatches = async () => {
    const data = await HttpService.get("/api/ops/batches");
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const batchData = await HttpService.get("/api/ops/batches");
        if (!mounted) return;
  ...
  const submitQuality = async () => {
    ...
    setSuccess("");
    try {
      await HttpService.post("/api/ops/quality-checks", {
  ...
  const createShipmentLot = async () => {
    ...
    try {
      ...
      await HttpService.post("/api/ops/batch-shipment-lots", {
        batchId: selectedBatch.id,
